...
export const detectMonument = async (imageUri: string): Promise<MonumentDetectionResult> => {
  try {
    ...
    // Read the image file as base64 (handle content:// URIs by copying to cache first)
    let base64: string | null = null;
    try {
      base64 = await FileSystem.readAsStringAsync(imageUri, { encoding: 'base64' });
    } catch (e) {
      try {
        const tmpPath = `${(FileSystem as any).cacheDirectory || (FileSystem as any).documentDirectory || ''}upload_${Date.now()}.jpg`;
        try {
          await (FileSystem as any).copyAsync({ from: imageUri, to: tmpPath });
        } catch (copyErr) {
          const dl = await (FileSystem as any).downloadAsync(imageUri, tmpPath);
          // dl.uri is the downloaded file
        }
        base64 = await FileSystem.readAsStringAsync(tmpPath, { encoding: 'base64' });
      } catch (e2) {
        console.error('[MonumentService] Fallback read failed:', e2);
